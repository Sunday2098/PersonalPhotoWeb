...
- "添加照片"按钮:多选图片上传(选择后立即检测大小,**超过 10MB 自动压缩**),
  自动读 EXIF 并生成数据文件,与 `add-photos` 脚本逻辑一致
- **项目封面**:悬停照片点 ▣ 一键设为该项目封面
- **首页 Hero 管理**:拖照片到左侧「🏠 首页 Hero 轮播」卡片即上首页主体轮播,
  进入该卡片可 ✕ 移出;**精选照片**:悬停照片点 ★ 或选中后"设为精选",
  上首页 12 张精选墙(带 精选/封面 徽标一眼可见)
- **删除照片**:照片悬停 ✕ 或选中后"删除所选"(连同 Cloudinary 云端图片一起删,
  不可恢复);**删除项目**:项目卡片悬停 ✕(项目里还有照片时会拦截,先移走再删)
- **"🚀 推送上线"按钮**:构建检查 → git commit → git push 全在界面里完成,
  实时显示执行日志(提交说明支持中文)
- 改动直接写入 `src/content/*.md`;服务只监听本机(127.0.0.1),
  不参与构建部署,摄影网站上完全看不到
